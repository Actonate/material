...
exports.buildJsExtended = buildJsExtended;
 ...
 * @param {boolean} isRelease Whether to build in release mode.
 */
function buildJsExtended() {
  var jsFiles = config.jsExtendedFiles;
  gutil.log("building extended js files...");
  ...
  var jsProcess = series(jsBuildStream, themeBuildStream())
    .pipe(concat('angular-material-extended.js'))
    .pipe(BUILD_MODE.transform())
    .pipe(insert.prepend(config.banner))
    .pipe(gulp.dest(config.outputDir))
    .pipe(gulpif(!IS_DEV, uglify({
...
function buildModule(module, opts) {
  ...
  function buildModuleStyles(name) {
    ...
    config.themeBaseFiles.forEach(function(fileGlob) {
      files = files.concat(glob(fileGlob, {
        cwd: ROOT
      }));
    });
